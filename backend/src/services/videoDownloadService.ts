...
export interface DownloadAndSaveToLocalResult {
  ...
  inputPath?: string;
  filename?: string;
  fileBaseName?: string; // Базовое имя файла (без расширения)
  videoId?: string;
  channelSlug?: string; // Для обратной совместимости
  channelName?: string;
  storage?: {
    filePath: string;
    resolvedPath?: string;
...
export async function downloadAndSaveToLocal(
  ...
  try {
    ...
    Logger.info(`downloadAndSaveToLocal [${mode}]: checking channel in Firestore`, {
      mode,
      userId,
      channelId,
      firestorePath
    ...
    Logger.info(`downloadAndSaveToLocal [${mode}]: channel check result`, {
      mode,
      userId,
      channelId,
      firestorePath,
    ...
    if (!channelSnap.exists) {
      Logger.warn(`downloadAndSaveToLocal [${mode}]: CHANNEL_NOT_FOUND`, {
        mode,
        userId,
        channelId,
        firestorePath,
    ...
    try {
      ...
      // Шаг 2: Используем новый StorageService
      const { getStorageService } = await import("./storageService");
      const { generateVideoId, generateUniqueVideoFileName, makeSafeBaseName } = await import("../utils/fileUtils");
      ...
      Logger.info(`downloadAndSaveToLocal [${mode}]: folder keys resolved`, {
        ...
        channelFolderKey,
        channelName: channelData.name || "not set",
        registrationEmail: userEmail, // userEmail уже получен выше
        videoTitle: videoTitle || "not provided"
      });
      ...
      const archiveDir = storage.resolveUploadedDir(userFolderKey, channelFolderKey);
        ...
        return {
          ...
          storage: {
            ...
            userDir,
            inputDir: storage.resolveInboxDir(userFolderKey, channelFolderKey),
            archiveDir,
            userEmail
          }
        };
