...
router.post("/:id/test-blottata", authRequired, async (req, res) => {
  ...
  try {
    ...
    if (fileId) {
      ...
      // ПРИМЕЧАНИЕ: Для Google Drive требуется driveInputFolderId, но мы перешли на локальное хранилище
      // Этот блок работает только если driveInputFolderId указан (для обратной совместимости)
      if (!channel.driveInputFolderId) {
        return res.status(400).json({
          error: "Google Drive folder not configured",
          message: "Для тестирования через Google Drive необходимо указать входную папку. Используйте локальное хранилище для автоматических путей."
        });
      }
    ...
    for (const channelData of channels) {
      try {
        ...
        const newChannelData: any = {
          name: finalName,
          initialName: finalName, // Сохраняем первичное название для стабильности папки
